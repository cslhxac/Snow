...
class CG_VECTOR:public KRYLOV_VECTOR_BASE<T>
{
    ...
    ARRAY<VECTOR<T,3>,int>& array;
public:
    CG_VECTOR(ARRAY<VECTOR<T,3>,int>& array_input) : array(array_input) {}

    static ARRAY<VECTOR<T,3>,int>& Array(BASE& base_array)
    {return ((CG_VECTOR&)(base_array)).array;}

    static const ARRAY<VECTOR<T,3>,int>& Array(const BASE& base_array)
    ...
    {ARRAY<VECTOR<T,3>,int>::Copy(c,Array(bv),array);}

    void Copy(const T c1,const BASE& bv1,const BASE& bv2)
    {ARRAY<VECTOR<T,3>,int>::Copy(c1,Array(bv1),Array(bv2),array);}
